...
class TranSupervisiController extends AdminController
{
  ...
  protected function grid()
  {
    ...
    $grid->actions(function ($actions) {
      ...
      if (Admin::user()->inRoles(['mitra'])) {
        if ($actions->row->task == 'PENGISIAN PLAN BY MITRA' || $actions->row->task == 'PENGISIAN ACTUAL' || $actions->row->task == 'ADMINISTRASI') {
          $actions->add(new Plan);
          if ($actions->row->task == 'PENGISIAN ACTUAL' || $actions->row->task == 'ADMINISTRASI') {
      ...
      if (Admin::user()->inRoles(['administrator', 'hd-ped', 'witel'])) {
    ...
    $grid->column('supervisi_waspang.name')->modal('Waspang', function ($model) {
      ...
      $form->select('waspang_id', 'Pilih Waspang')->options(
        MstWaspangUt::join('admin_role_users', 'admin_users.id', '=', 'admin_role_users.user_id')
  ...
  protected function detail($id)
  {
    ...
    return view('admin.modul_project.detail-supervisi', [
      'data' => $data,
      'supervisi' => $supervisi,
      'tgl_ct' => $tgl_ct,
      'tgl_ut' => $tgl_ut,
  ...
  public function baseLineActivityAdd(Request $request)
  {
    ...
    //   //echo $_POST['volume'][8];
    //   foreach ($_POST['volume'] as $row => $value) {
    //     echo $_POST['activity_id'][$row];
    //     echo '<br>';
    //   }
